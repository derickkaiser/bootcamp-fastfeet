...
class DeliveryController {
    async store(req, res){
        ...
        const { recipient_id, deliveryman_id, signature_id,
                product_name, start_date, end_date,
                canceled_at } = req.body;

        const recipientExists = await Recipient.findOne({ where: { id: recipient_id }});

        if(!recipientExists){
            return res.status(400).json({ error: 'Recipient does not exist.' });
        }

        const deliverymanExists = await Deliveryman.findOne({ where: { id: deliveryman_id }});
        ...
        const delivery = await Delivery.create(
            { recipient_id, deliveryman_id, signature_id, product_name,
                ...
                include: [
                    {
                        model: Recipient,
                        as: 'recipient',
                        attributes: ['id','name', 'street', 'city', 'state', 'cep', 'number'],
                    },
                    {
                        model: Deliveryman,
                        as: 'deliveryman',
    ...
    async update(req, res){
        ...
        if(!delivery){
            return res.status(401).json({ error: `Delivery with id ${id} does not exist.` });
        }
    ...
    async delete(req, res){
        ...
        if(!delivery){
            return res.status(401).json({ error: `Delivery with ${id} does not exist.`});
        }
